...
counter = 0
g_out = np.zeros(shape=(len(density) * len(rad), 5, 470))
ssa_out = np.zeros(shape=(len(density) * len(rad), 5, 470))
tau_out = np.zeros(shape=(len(density) * len(rad), 5, 470))
alb_out = np.zeros(shape=(470, len(density) * len(rad)))
total_runs = (len(density) * len(rad))
result = np.zeros(shape=(len(density),len(rad),470))
...
for i in np.arange(0,len(density),1):
    for j in np.arange(0,len(rad),1):
        ...
        umu0 = 0.55 # cosine of solar zenith angle 
        ...
        tempData = pd.DataFrame({"rho":rho_snw[0], "rds":rds_snw[0], "BBA":BBA},index=[counter])
        outData = outData.append(tempData)

        result[i,j,:] = alb_out[:,counter]
...
np.save(str(save_path+'LUT_clean.npy'),result)